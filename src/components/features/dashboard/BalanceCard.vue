<template>
  <div class="group relative bg-white dark:bg-gray-800 rounded-xl sm:rounded-2xl shadow-lg hover:shadow-2xl p-3 sm:p-5 md:p-6 lg:p-7 border hover-lift overflow-hidden transition-all duration-300"
    :class="borderClass"
  >
    <div class="relative flex items-center justify-between gap-2">
      <div class="flex-1 min-w-0">
        <p class="text-[10px] sm:text-xs md:text-sm font-semibold mb-1 sm:mb-2 flex items-center gap-1" :class="labelClass">
        ...
        <p class="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-extrabold break-words" :class="valueClass">
          ${{ balance.toLocaleString('es-CO') }}
        </p>
        <p class="text-[9px] sm:text-[10px] md:text-xs mt-1 sm:mt-2 font-medium" :class="subtitleClass">
          {{ balanceStatus }}
        </p>
      </div>
